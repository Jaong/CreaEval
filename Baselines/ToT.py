...
def ToT_Step_Score(model_name, FS_num, step_num, response_content, Model_LST):
    ...
    for Score_LLM in Model_LST[:-1]:
        ...
        with open(r"text_file\ToT_Score_LLM_User_Step-" + str(step_num) + "_prompt.txt", "w", encoding="utf-8") as f:
            f.write(Score_user_prompt)
    ...
    Score_lst.append(true_response)
...
def ToT_Score(Model_name, ans_id, single_response, file_core_name, temperature):
...
if __name__ == "__main__":
    ...
    for ans_id in Pure_responses:
        ToT_Score(model_name, ans_id, Pure_responses[ans_id], 'ToT', temperature)